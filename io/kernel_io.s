...
__hardware:
		# Catch receiver/transmitter interrupt
		lui		t0, 0xbfa0			# Place interrupt I/O port adress in t0
		lbu     t0, 0x0(t0)			# Load t0 with the contents of the adress
		
		andi	t0, t0, 32			# Mask out K1 occured bit
		bnez	t0, __receive		# Receiver interrupt
		
__hardware_K2:
		lui		t0, 0xbfa0			# Place interrupt I/O port adress in t0
		lbu     t0, 0x0(t0)			# Load t0 with the contents of the adress
		
		andi	t0, t0, 16			# Mask out K2 occured bit
		bnez	t0, __transmit		# Transmitter interrupt
...
__hardware_timer:		
		...
		andi	t0, t0, 0x40		# Mask out escape occured bit
		bnez	t0, __external_timer_handler
		
		b		__really_ret

###########################################################################
# Buffers to keep characters received or to transmit 

		.data						# Should be .kdata
__inbuf:
		.space	16
__input:
		.word	0					# Index for next empty place in inbuf
__inget:
		.word	0					# Index for oldest character in inbuf
 
__utbuf:
		.space	16
__utput:
		.word	0					# Index for next empty place in utbuf
__utget:
		.word	0					# Index for oldest character in utbuf
		.text						# Should be .ktext

###########################################################################
# receive: fetch character from receiver and put in inbuf
#
 
__receive:
		la		t0, BUTTONS			# Place adress of buttons in t0
		lb		t1, 0x0(t0)			# Load button port value
		andi	t1, t1, 2			# Mask out button indication bits
		bne		t1, zero, __receive	# Loop if button pressed
		
		la		t0, SWITCHES		# Place adress of switches in t0
		lb		t0, 0x0(t0)			# Load switch position
		
		lw		t1, __input
		sb		t0, __inbuf(t1)		# Store in inbuf
		
		addi	t1, t1, 1			# Point to next empty space
		bne		t1, 16,  __rwrap	# Check if wrap-around
		li		t1, 0
__rwrap:
		lw		t0, __inget			# Check if buffer full
		beq		t1, t0, __rfull
		sw		t1, __input			# Only update if not full
__rfull:
		
		b		__hardware_K2
 
###########################################################################
# transmit: transmitter needs more characters - if outbuf not empty
#           send a new character otherwise: shut down transmitter IRQ
#
 
__transmit:
		la		t0, BUTTONS			# Place adress of buttons in t0
		lb		t1, 0x0(t0)			# Load button port value
		andi	t1, t1, 1			# Mask out button indication bits
		bne		t1, zero, __transmit	# Loop if button pressed

		lw		t0, __utget
		lw		t1, __utput
		beq		t0, t1, __tend		# Buffer empty -> return
		
		lbu		t1, __utbuf(t0)		# Get switch position
        sb		t1, LEDS			# Output switch position to LEDs
		
		addi	t0, t0, 1			# Point to next char in utbuf
		bne		t0, 16, __twrap		# Check if wrap-around
		li		t0, 0
__twrap:
		sw		t0, __utget
__tend:
...
###########################################################################
# getchar (syscall 22): Return a character from inbuf
#
# Returns:  $v0 = 0 if no character, 1 if character
#           $v1 = the character when $v0 = 1
 
__getchar:
		li		t0, 0
		sw		t0, __s_v0			# Set saved v0 to 0
		
		lw		t0, __inget
		lw		t1, __input
		beq		t0, t1, __inempty	# Buffer empty -> get out
		
		lbu		t1, __inbuf(t0)
		sw		t1, __s_v1			# Set saved v1 to character
		li		t1, 1
		sw		t1, __s_v0			# Set saved v0 to 1
		
		addi	t0, t0, 1			# Point to next char in inbuf
		bne		t0, 16, __wrap22	# Check if wrap-around
		li		t0, 0
__wrap22:
		sw		t0, __inget
__inempty:
...
###########################################################################
# putchar (syscall 23): Output a character to transmitter
#
# Args: $a0 = the character
 
__putchar:
		lw		t0, __s_a0			# Fetch character to output
		lw		t1, __utput
		sb		t0, __utbuf(t1)		# Store in utbuf
		
		addi	t1, t1, 1			# Point to next empty space
		bne		t1, 16, __wrap23	# Check if wrap-around
		li		t1, 0
__wrap23:
		lw		t0, __utget			# Check if buffer full
		beq		t1, t0, __end23
		sw		t1, __utput			# Only update if not full
__end23:
		b     __ret
